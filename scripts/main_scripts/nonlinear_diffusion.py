...
from separable_operator_networks.pde import nonlinear_diffusion
from separable_operator_networks.train import train_loop, update
from separable_operator_networks.eval import (
    eval_nonlinear_diffusion,
    pred_nonlinear_diffusion,
)
...
parser.add_argument(
    "--results-dir",
    type=str,
    default="../../results/nonlinear_diffusion/",
...
parser.add_argument(
    "--hidden", type=int, default=128, help="the size of each hidden layer"
)
parser.add_argument(
    "--r",
    type=int,
...
fs_train = jnp.load(args.data_dir + "fs_train.npy").reshape(-1, 101**2)
fs_test = jnp.load(args.data_dir + "fs_test.npy").reshape(-1, 101**2)
...
root_dir = os.path.join(
    args.results_dir,
    "results",
    "results_nonlinear_diffusion",
    args.model_name,
)
result_dir = os.path.join(
    root_dir,
    "nf" + str(args.batch) + "_nc" + str(args.nc) + "_r" + str(args.r),
)
...
if args.model_name == "DeepONet":
    train_generator = jax.jit(
        lambda key: nonlinear_diffusion.DeepONet_train_generator_nonlinear_diffusion(
            fs_train, args.batch, args.nc, key
        )
    )
    test_generator = jax.jit(
        nonlinear_diffusion.DeepONet_test_generator_nonlinear_diffusion
    )
    loss_fn = nonlinear_diffusion.apply_model_DeepONet
elif args.model_name == "SepONet":
    train_generator = jax.jit(
        lambda key: nonlinear_diffusion.SepONet_train_generator_nonlinear_diffusion(
            fs_train, args.batch, args.nc, key
        )
    )
    test_generator = jax.jit(
        nonlinear_diffusion.SepONet_test_generator_nonlinear_diffusion
    )
    loss_fn = nonlinear_diffusion.apply_model_SepONet
...
rel_l2_mean, rel_l2_std, rmse_mean, rmse_std = eval_nonlinear_diffusion(
...
basis_ts, basis_b, u_pred = pred_nonlinear_diffusion(
