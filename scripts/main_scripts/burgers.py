...
from separable_operator_networks.pde import burgers
from separable_operator_networks.train import train_loop, update
from separable_operator_networks.eval import eval_Burgers, pred_Burgers
...
fs_train = jnp.array(scipy.io.loadmat(args.data_dir + "GRFs.mat")["saved_GRFs"])
fs_test = jnp.array(scipy.io.loadmat(args.data_dir + "Burger.mat")["input"])
u_test = jnp.array(
    scipy.io.loadmat(args.data_dir + "Burger.mat")["output"]
)  # check u_test[:,0,:]-fs_test ==0

# result dir
root_dir = os.path.join(
    args.results_dir, "results", "results_Burgers", args.model_name
)
...
if args.model_name == "DeepONet":
    train_generator = jax.jit(
        lambda key: burgers.DeepONet_train_generator_Burgers(
            fs_train, args.batch, args.nc, key
        )
    )
    test_generator = jax.jit(burgers.DeepONet_test_generator_Burgers)
    loss_fn = burgers.apply_model_DeepONet
elif args.model_name == "SepONet":
    train_generator = jax.jit(
        lambda key: burgers.SepONet_train_generator_Burgers(
            fs_train, args.batch, args.nc, key
        )
    )
    test_generator = jax.jit(burgers.SepONet_test_generator_Burgers)
    loss_fn = burgers.apply_model_SepONet
...
rel_l2_mean, rel_l2_std, rmse_mean, rmse_std = eval_Burgers(
...
basis_ts, basis_b, u_pred = pred_Burgers(
