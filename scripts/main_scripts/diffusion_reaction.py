...
from separable_operator_networks.eval import eval_DR, pred_DR
...
parser.add_argument(
    "--results-dir",
    type=str,
    default="../../results/diffusion_reaction/",
...
parser.add_argument(
    "--hidden", type=int, default=50, help="the size of each hidden layer"
)
parser.add_argument(
    "--r",
    type=int,
...
root_dir = os.path.join(
    os.getcwd(), "results", "results_DR_pred", args.model_name
)
...
if args.model_name == "DeepONet":
    train_generator = jax.jit(
        lambda key: diffusion_reaction.DeepONet_train_generator_DR(
            fs_train, args.batch, args.nc, key
        )
    )
    test_generator = jax.jit(diffusion_reaction.DeepONet_test_generator_DR)
    loss_fn = diffusion_reaction.apply_model_DeepONet
elif args.model_name == "SepONet":
    train_generator = jax.jit(
        lambda key: diffusion_reaction.SepONet_train_generator_DR(
            fs_train, args.batch, args.nc, key
        )
    )
    test_generator = jax.jit(diffusion_reaction.SepONet_test_generator_DR)
    loss_fn = diffusion_reaction.apply_model_SepONet
...
rel_l2_mean, rel_l2_std, rmse_mean, rmse_std = eval_DR(
...
basis_ts, basis_b, u_pred = pred_DR(
