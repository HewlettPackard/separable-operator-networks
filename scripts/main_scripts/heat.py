...
from separable_operator_networks.eval import eval_heat, pred_heat
...
root_dir = os.path.join(
    args.results_dir, "results", "pred_heat_new", args.model_name
)
...
if args.model_name == "DeepONet":
    train_generator = jax.jit(
        lambda key: heat.DeepONet_train_generator_heat(
            fs_train, args.batch, args.nc, key
        )
    )
    test_generator = jax.jit(heat.DeepONet_test_generator_heat)
    loss_fn = heat.apply_model_DeepONet
elif args.model_name == "SepONet":
    train_generator = jax.jit(
        lambda key: heat.SepONet_train_generator_heat(
            fs_train, args.batch, args.nc, key
        )
    )
    test_generator = jax.jit(heat.SepONet_test_generator_heat)
    loss_fn = heat.apply_model_SepONet
...
rel_l2_mean, rel_l2_std, rmse_mean, rmse_std = eval_heat(
...
basis_ts, basis_b, u_pred = pred_heat(
